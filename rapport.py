...
Original file is located at
    https://colab.research.google.com/drive/1DaLvNIwyJXu0RjMPASvuz0wLnZl52-le
...
gray_like = build_image_like_tensor(16,16,3,126)
...
def target_to_one_hot(targets: torch.Tensor, num_classes=10) -> torch.Tensor:
    ...
    # YOUR CODE HERE 
    one_hot_matrix = torch.zeros((targets.shape[0], 10))
...
def convolution_forward_torch(image, kernel):
    # YOUR CODE HERE 
    n,m=np.shape(image)
    input = torch.zeros(1,1,n,m)
    for i in range(0,n): 
      input[0,0,i,:] = torch.from_numpy(image[i,:])
    m = nn.Conv2d(1,1,kernel.shape,padding=1,stride=1)
    return m(input)
...
#Display the 10 image from train set
display_10_images(fmnist_train.dataset)

#Display the 10 image from validation set
...
def fashion_mnist_dataset_answer():
    ...
    number_of_images_in_train_set = len(fmnist_train)*32
    number_of_images_in_test_set = len(fmnist_val)*32
